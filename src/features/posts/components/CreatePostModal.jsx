...
export default function CreatePostModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (isError) {
      const msg =
        error?.data?.message || error?.error || "Creating Post failed";
      toast.error(msg);
    }
  }, [isError, error]);
  ...
  const handleImageChange = (e) => {
    ...
    setValue("images", [...selectedFiles, ...validFiles]);
  };
  ...
  const addTagInput = () => setTagInputs([...tagInputs, ""]);

  const updateTagInput = (index, value) => {
  ...
  const submitForm = async (data) => {
    ...
    if (data.tags) {
      data.tags.forEach((tag) => formData.append("tags[]", tag.toLowerCase()));
    }
        ...
        <form className="space-y-4" onSubmit={handleSubmit(submitForm)}>
          <input
            {...register("title", {
              ...
              maxLength: {
                value: 100,
                message: t("postTitleMaxLength"),
              },
            })}
            placeholder={t("title")}
            className="w-full border border-gray-300 p-3 rounded-xl mb-2"
          />
          ...
          <textarea
            {...register("description", {
              ...
              minLength: {
                value: 5,
                message: t("postDescriptionMinLength"),
              },
            })}
            placeholder={t("whatsOnYourMind")}
            className="w-full border border-gray-300 p-3 rounded-xl h-28 resize-none mb-2"
          />
          {errors.description && (
          ...
          <div className="space-y-3  p-4 rounded-xl bg-gray-50">
            <h3 className="font-semibold text-gray-700">{t("postLocation")}</h3>
            ...
            <div className="space-y-3 mt-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-green-500 text-white rounded-xl w-full cursor-pointer"
              >
                {t("useThisLocation")}
              </button>

              {locationLoading && (
                <p className="text-blue-600">{t("loadingLocation")}...</p>
              )}

              <button
                type="button"
                onClick={() => {
                  // setValue("address", userSavedAddress);
                }}
                className="px-4 py-2 bg-gray-200 rounded-xl w-full cursor-pointer"
              >
                {t("useMySavedLocation")}
              </button>
            </div>
          </div>
          ...
          {tagInputs.length === 10 && (
            <InputError message={t("maxTagsReached")} />
          )}
          ...
          <div className="flex justify-end items-center gap-3 pt-2">
            <button
              ...
              className="px-5 py-2 bg-gray-200 cursor-pointer w-[30%] h-14 rounded-xl"
            >
              {t("cancel")}
            </button>

            <LoadingButton
              disabled={!isValid || !coords || selectedFiles.length === 0}
              title={t("send")}
              width="70%"
